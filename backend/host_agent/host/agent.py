...
class HostAgent:
    ...
    def get_investment_amount(self):
        """Returns the stored investment amount with appropriate currency symbol."""
        state = self._load_state()
        if state["investment_amount"] > 0:
            # Use rupee symbol for India market, dollar for US
            currency_symbol = "₹" if state.get("market_preference") == "INDIA" else "$"
            return f"Current investment amount: {currency_symbol}{state['investment_amount']:,.2f}"
        else:
...
Return ONLY a JSON array of uppercase ticker symbols. If a name is already a ticker, keep it as is."""
    ...
    def get_stock_lists(self):
        ...
        if state["investment_amount"] > 0:
            result += f"{currency_symbol}{state['investment_amount']:,.2f}\n\n"
        else:
            result += "Not set yet.\n\n"
    ...
    def analyze_all_stocks(self):
        ...
        **INVESTMENT APPROACH:**
        ...
        # Include user ID, session ID, and market preference in the delegation request
        session_id = self.current_session_id.get("id", "unknown")
        user_id = self.current_session_id.get("user_id", "unknown")
        market_preference = state.get("market_preference", "US")
