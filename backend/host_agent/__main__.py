...
# Custom rate limit key function that handles CloudFront/proxy headers
def get_real_client_ip(request: Request) -> str:
    """
    Extract the real client IP from X-Forwarded-For header (set by CloudFront/ALB).
...
# Uses real client IP from X-Forwarded-For header (behind CloudFront/ALB)
...
class AsyncChatResponse(BaseModel):
    """Response model for async chat submission."""
    task_id: str
    session_id: str
...
# Redis client for async task tracking
...
async def init_redis():
    """Initialize Redis client for async task tracking."""
    global redis_client
    redis_url = os.getenv("REDIS_URL")

    # If no REDIS_URL is set and we're in local mode, use local Redis
    if not redis_url and Config.is_local():
        redis_url = "redis://localhost:6379"
        logger.info("[REDIS] No REDIS_URL set, using local Redis at localhost:6379")

    if redis_url:
        # Log the Redis URL (mask any password if present)
        safe_url = redis_url.split("@")[-1] if "@" in redis_url else redis_url
        logger.info(f"[REDIS] Attempting to connect to Redis at: {safe_url}")
        try:
            redis_client = redis.from_url(redis_url, decode_responses=True)
            logger.info(f"[REDIS] Redis client created, attempting ping...")
            await redis_client.ping()
            logger.info(f"[REDIS] Redis client initialized successfully - PING successful")
        except Exception as e:
            logger.error(f"[REDIS] Failed to connect to Redis: {e}. Async endpoints will be unavailable.")
            import traceback
            logger.error(f"[REDIS] Traceback: {traceback.format_exc()}")
            redis_client = None
    else:
        logger.warning("[REDIS] REDIS_URL not set and not in local mode. Async endpoints will be unavailable.")
...
def init_firebase():
    ...
    try:
        ...
        # Try to use service account JSON from environment variable first (for AWS Secrets Manager)
        ...
        if service_account_json:
            try:
                ...
                logger.info(f"Firebase Admin SDK initialized with service account JSON for project: {project_id}")
                return
...
def validate_firebase_token(id_token: str) -> dict:
    ...
    In production, fails hard if Firebase is not configured.
    In development, falls back to dev mode for testing.
    """
    ...
    try:
        ...
        if not firebase_admin._apps:
            # In production, fail hard - do not allow dev fallback
            if Config.is_production():
                logger.error("[TOKEN] Firebase not initialized in production - rejecting request")
                raise HTTPException(
                    status_code=503,
        ...
        # In production, fail hard - do not allow dev fallback
        if Config.is_production():
            logger.error("[TOKEN] Token validation failed in production - rejecting request")
            raise HTTPException(
                status_code=401,
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Initialize Redis for async task tracking
        await init_redis()
...
@limiter.limit("10/minute")  # Strict limit for login to prevent brute force
...
@limiter.limit("40/minute")  # Rate limit chat messages
...
@limiter.limit("30/minute")  # Rate limit streaming chat messages
...
# =====================================================
# ASYNC CHAT ENDPOINTS (for long-running requests)
# =====================================================

async def process_chat_task(task_id: str, session_id: str, user_id: str, message: str, paid_user: bool):
    """Background task to process chat message and store result in Redis."""
    global redis_client, host_agent_instance

    if not redis_client:
        logger.error(f"Redis client not available for task {task_id}")
        return

    try:
        # Update status to processing
        logger.info(f"[REDIS] Setting task:{task_id} status to 'processing'")
        await redis_client.hset(f"task:{task_id}", mapping={
            "status": "processing",
            "progress": "Starting portfolio analysis..."
        })
        logger.info(f"[REDIS] Successfully set task:{task_id} to 'processing'")

        # Get database session
        db = next(get_db())

        try:
            logger.info(f"[ASYNC] Processing task {task_id} for session {session_id}")
            ...
            # Update progress
            await redis_client.hset(f"task:{task_id}", "progress", "Analyzing portfolio data...")
            ...
            async for response_chunk in host_agent_instance.stream(
                ...
                if response_count % 10 == 0:
                    await redis_client.hset(f"task:{task_id}", "progress", f"Processing... ({response_count} steps)")

            # Add agent response to database
            if full_response:
                add_message(db, session_id, user_id, "agent", full_response, "host_agent")
            ...
            try:
                ...
                logger.error(f"Error saving agent state for task {task_id}: {e}")

            # Check file upload status (from DB or from the initial Redis state)
            session_obj = get_session(db, session_id)
            is_file_uploaded = session_obj.portfolio_statement_uploaded if session_obj else False
            if not is_file_uploaded:
                task_data = await redis_client.hget(f"task:{task_id}", "file_uploaded")
                is_file_uploaded = task_data == "true"

            # Store completed result in Redis
            logger.info(f"[REDIS] Storing completed result for task:{task_id}")
            await redis_client.hset(f"task:{task_id}", mapping={
                "status": "completed",
                "response": full_response,
                "is_complete": "true" if is_complete else "false",
                "is_file_uploaded": "true" if is_file_uploaded else "false",
                "end_session": "true" if end_session else "false",
                "progress": "Complete"
            })
            logger.info(f"[REDIS] Successfully stored completed result for task:{task_id}")
...
@app.post("/api/chats/async", response_model=AsyncChatResponse)
@limiter.limit("40/minute")
async def chat_async(request: Request, current_user: dict = Depends(get_current_user)):
    """
    Submit a chat message for async processing.
    Returns immediately with a task_id that can be polled for results.
    Use this for long-running operations like file uploads to avoid gateway timeouts.
    ...
    if redis_client is None:
        raise HTTPException(
            status_code=503,
            detail="Async processing not available. Redis not configured."
        )

    if host_agent_instance is None:
        raise HTTPException(status_code=503, detail="Host Agent not initialized")

    # Parse request - same logic as /api/chats
    content_type = request.headers.get("content-type", "")
    ...
    if "application/json" in content_type:
        ...
        try:
            ...
            user_id = form.get("user_id")
            paid_user = form.get("paid_user")
            session_id_form = form.get("session_id")
            uploaded_file = form.get("file")
            ...
            final_message = message
            final_user_id = user_id
            final_session_id = session_id_form
            final_paid_user = paid_user == "true" if paid_user is not None else False
            logger.info(f"[ASYNC] Using form data input format")
        except HTTPException:
            raise
        except Exception as e:
            raise HTTPException(status_code=400, detail=f"Invalid form data: {str(e)}")
    ...
    try:
        # Validate user and session (same checks as sync endpoint)
        user = get_or_create_user(db, final_user_id, paid_user=final_paid_user)
        ...
        if not session:
            user_email = user.email if user.email else "no-email@example.com"
            can_generate, limit_message, _, _ = can_user_generate_report(db, user_email, final_user_id)
            if not can_generate:
        ...
        # Handle file upload synchronously (before background task)
        file_uploaded = False
        if uploaded_file and hasattr(uploaded_file, 'filename') and uploaded_file.filename:
            # Check if session can accept file upload
            can_upload, upload_error = can_session_upload_file(db, session_id, final_user_id)
            if not can_upload:
            ...
            try:
                ...
                try:
                    ...
                    if "successfully" in result:
                        host_agent_instance.current_session_id["is_file_uploaded"] = True
                        file_uploaded = True
                        logger.info(f"[ASYNC] Portfolio file uploaded successfully for user {final_user_id}, session {session_id}")
                    else:
                        logger.error(f"[ASYNC] Portfolio file upload failed: {result}")
                finally:
                    import os
                    try:
                        os.unlink(temp_file_path)
                ...
                logger.error(f"[ASYNC] Error processing uploaded file: {e}")
                raise HTTPException(status_code=500, detail=f"Error processing uploaded file: {str(e)}")
        ...
        decrement_user_credits(db, final_user_id)

        # Generate task ID
        task_id = str(uuid.uuid4())

        # Store initial task state in Redis
        logger.info(f"[REDIS] Creating task:{task_id} in Redis for session {session_id}")
        await redis_client.hset(f"task:{task_id}", mapping={
            "status": "pending",
            "session_id": session_id,
            "user_id": final_user_id,
            "file_uploaded": "true" if file_uploaded else "false",
            "progress": "Task queued"
        })
        await redis_client.expire(f"task:{task_id}", TASK_EXPIRATION_SECONDS)
        logger.info(f"[REDIS] Successfully created task:{task_id} with TTL {TASK_EXPIRATION_SECONDS}s")

        # Start background task
        asyncio.create_task(
            process_chat_task(task_id, session_id, final_user_id, final_message, final_paid_user)
        )

        logger.info(f"[ASYNC] Created task {task_id} for session {session_id}")

        return AsyncChatResponse(
            task_id=task_id,
            session_id=session_id,
            status="pending",
            message="Task submitted. Poll /api/chats/tasks/{task_id} for results."
        )
...
@app.get("/api/chats/tasks/{task_id}", response_model=TaskStatusResponse)
async def get_task_status(task_id: str, current_user: dict = Depends(get_current_user)):
    ...
    return TaskStatusResponse(
        ...
        is_file_uploaded=task_data.get("is_file_uploaded") == "true",
        end_session=task_data.get("end_session") == "true",
...
@limiter.limit("5/minute")  # Strict limit for payment verification
