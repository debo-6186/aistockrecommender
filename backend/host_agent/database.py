...
def add_user_message_credits(db: Session, user_id: str, credits_to_add: int = 30) -> tuple[bool, int]:
    """
    Add message credits to a user account.
    ...
    try:
        ...
        logger.info(f"Added {credits_to_add} message credits to user {user_id}. New total: {user.message_credits}")
        return True, user.message_credits
    except SQLAlchemyError as e:
        db.rollback()
        logger.error(f"Error adding message credits to user {user_id}: {e}")
        return False, 0
...
def save_stock_recommendation(db: Session, session_id: str, user_id: str, recommendation: dict) -> Optional[StockRecommendation]:
    """Save stock recommendation to database and decrement user's report credits."""
    try:
        ...
        # Decrement user's report credits after successful save
        if user.email:
            decrement_user_report_credits(db, user.email)
        else:
            logger.warning(f"User {user_id} has no email, cannot decrement report credits")
...
def can_user_generate_report(db: Session, email: str, user_id: str) -> tuple[bool, str, int, int]:
    ...
    try:
        ...
        # Check if max_reports is -1 (unlimited for paid users)
        if whitelist_entry.max_reports == -1:
            logger.info(f"User {email} has unlimited reports (paid user)")
            return True, "", 0, -1

        # Check if user has no reports remaining (max_reports == 0)
        if whitelist_entry.max_reports <= 0:
            message = (
                "You have no report credits remaining. "
                "Please add credits to generate more reports."
            )
            logger.warning(f"User {email} has no report credits remaining (max_reports={whitelist_entry.max_reports})")
            return False, message, 0, whitelist_entry.max_reports

        # User can generate reports (max_reports > 0 means remaining credits)
        logger.info(f"User {email} can generate {whitelist_entry.max_reports} more report(s)")
        return True, "", 0, whitelist_entry.max_reports
...
def add_user_credits(db: Session, email: str, additional_credits: int) -> bool:
    ...
    try:
        ...
        # Add credits to max_reports (unless it's already unlimited, i.e. -1)
        if whitelist_entry.max_reports == -1:
            logger.info(f"User {email} already has unlimited reports")
            return True

        # Add credits (works for both 0 and positive values)
...
def decrement_user_report_credits(db: Session, email: str) -> bool:
    """
    Decrement report credits by 1 when a report is generated.
    ...
    try:
        ...
        if not whitelist_entry:
            logger.error(f"Whitelist entry not found for {email}")
            return False

        # Don't decrement if user has unlimited reports (-1)
        if whitelist_entry.max_reports == -1:
            logger.info(f"User {email} has unlimited reports, not decrementing")
            return True

        # Don't decrement below 0
        if whitelist_entry.max_reports <= 0:
            logger.warning(f"User {email} has no report credits to decrement")
            return False

        # Decrement report credits
        whitelist_entry.max_reports -= 1
        whitelist_entry.updated_at = datetime.utcnow()
        db.commit()
        logger.info(f"Decremented report credits for {email}. Remaining: {whitelist_entry.max_reports}")
        return True

    except SQLAlchemyError as e:
        db.rollback()
        logger.error(f"Error decrementing report credits for {email}: {e}")
        return False
