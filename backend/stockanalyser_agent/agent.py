...
class StockAnalyzerAgent:
    ...
    def __init__(self):
        ...
        self.market_preference = "US"  # Track market preference (US or INDIA)
...
MARKET_PREFERENCE: US or INDIA (extract from MARKET PREFERENCE field, default to US if not found)

EXAMPLES:
Input: "I have 10 shares of Apple and 5 shares of Microsoft in my portfolio. I want to invest $5000 in Tesla and Amazon. Email: john@example.com MARKET PREFERENCE: US"
Output:
EXISTING: AAPL, MSFT
NEW: TSLA, AMZN
...
Input: "Analyze my NVDA (20 shares), VOO (15.5 shares) and suggest new stocks PLTR, GOOGL for $10000. USER ID: user123 SESSION ID: sess456 MARKET PREFERENCE: US"
Output:
EXISTING: NVDA, VOO
NEW: PLTR, GOOGL
...
Input: "I own RELIANCE.NS, TCS.NS. Want to invest 50000 in INFY.NS and HDFCBANK.NS. MARKET PREFERENCE: INDIA"
Output:
EXISTING: RELIANCE.NS, TCS.NS
NEW: INFY.NS, HDFCBANK.NS
SHARES: NONE
...
MARKET_PREFERENCE: INDIA"""
            ...
            if response and response.text:
                ...
                for line in lines:
                    ...
                    elif line.startswith("MARKET_PREFERENCE:"):
                        market_pref = line.replace("MARKET_PREFERENCE:", "").strip().upper()
                        if market_pref in ["US", "INDIA"]:
                            self.market_preference = market_pref
                        else:
                            self.market_preference = "US"  # Default to US
            ...
            logger.info(f"Market preference: {self.market_preference}")
    ...
    def get_expert_portfolio_recommendations(self, analysis_request: str = "") -> str:
        ...
        """
        try:
            # Determine currency symbol based on market preference
            currency_symbol = "₹" if self.market_preference == "INDIA" else "$"
            logger.info(f"get_expert_portfolio_recommendations called with investment amount: {currency_symbol}{self.investment_amount}, market: {self.market_preference}")
...
MARKET CONTEXT:
- Market: {self.market_preference} ({'Indian stocks on NSE/BSE' if self.market_preference == 'INDIA' else 'US stocks on NYSE/NASDAQ'})
- Currency: {currency_symbol} ({'Indian Rupees (INR)' if self.market_preference == 'INDIA' else 'US Dollars (USD)'})
- **IMPORTANT**: All monetary values in your response MUST use the {currency_symbol} symbol
...
4. If NO stocks qualify for BUY (all are HOLD/SELL):
   ...
   - Provide "cash_reserve_recommendation": "{currency_symbol}X (no qualified investments found)"
   - Suggest user expand stock universe or adjust criteria

PORTFOLIO CONSTRAINTS:
- Total investment budget: {currency_symbol}{self.investment_amount}
...
- **Allocation examples: {currency_symbol}2500, {currency_symbol}2250, {currency_symbol}2000 (for {currency_symbol}10k budget)**
...
- **Allocation examples: {currency_symbol}1500, {currency_symbol}1250, {currency_symbol}1000 (for {currency_symbol}10k budget)**
...
- **Allocation examples: {currency_symbol}750, {currency_symbol}600, {currency_symbol}500 (for {currency_symbol}10k budget)**

⚠️ **WEIGHTAGE DISTRIBUTION RULE:**
If you have 3+ BUY recommendations, they MUST have DIFFERENT allocation amounts.
Example CORRECT allocations for {currency_symbol}10,000 budget with 4 BUY stocks:
- Stock A (HIGH): {currency_symbol}2,500 (25%)
- Stock B (MEDIUM-HIGH): {currency_symbol}2,000 (20%)
- Stock C (MEDIUM): {currency_symbol}1,500 (15%)
- Stock D (LOW-MEDIUM): {currency_symbol}1,000 (10%)
- Remaining: {currency_symbol}3,000 saved or allocated to top picks

Example INCORRECT (will be rejected):
- All stocks: {currency_symbol}2,500 each ❌ (equal distribution shows no differentiation)

ALLOCATION RULES (** CRITICAL - MUST FOLLOW **):
1. ⚠️ NEVER EVER allocate {currency_symbol}0 to a BUY recommendation - minimum is 5% of total budget
2. ⚠️ If a stock CANNOT be allocated due to ANY constraint (sector limits, budget, etc.), mark it as HOLD, NOT BUY
3. Total BUY allocations MUST sum exactly to {currency_symbol}{self.investment_amount}
...
6. For HOLD and SELL: investment_amount is always "{currency_symbol}0"
...
1. Sector constraint: If NFLX looks good but you already have 40% in Communication Services sector:
   ❌ WRONG: {{"ticker": "NFLX", "recommendation": "BUY", "investment_amount": "{currency_symbol}0"}}
   ✅ CORRECT: {{"ticker": "NFLX", "recommendation": "HOLD", "investment_amount": "{currency_symbol}0", "reasoning": "Strong fundamentals but sector concentration limit prevents allocation"}}

2. Missing share count: If AAPL should be sold but share count is not provided:
   ❌ WRONG: {{"ticker": "AAPL", "recommendation": "SELL", "investment_amount": "{currency_symbol}0"}}
   ✅ CORRECT: {{"ticker": "AAPL", "recommendation": "HOLD", "investment_amount": "{currency_symbol}0", "reasoning": "Overvalued and should be sold, but share count not provided. Cannot make SELL recommendation without knowing position size."}}

3. SELL with share count provided: If TSLA should be sold and you know user owns 20 shares:
   ❌ WRONG: {{"ticker": "TSLA", "recommendation": "SELL", "investment_amount": "{currency_symbol}0", "reasoning": "Overvalued"}}
   ✅ CORRECT (Complete Exit): {{"ticker": "TSLA", "recommendation": "SELL", "investment_amount": "{currency_symbol}0", "shares_to_sell": "ALL (20 shares)", "reasoning": "Significantly overvalued at 30% above analyst target with deteriorating fundamentals. Recommend complete exit of all 20 shares."}}
   ✅ CORRECT (Partial): {{"ticker": "TSLA", "recommendation": "SELL", "investment_amount": "{currency_symbol}0", "shares_to_sell": "PARTIAL: 10 shares", "reasoning": "Moderately overvalued. Recommend selling 50% (10 of 20 shares) to reduce concentration risk while maintaining some exposure."}}
...
{{
    "allocation_breakdown": [
        {{
            ...
            "investment_amount": "string (e.g., '{currency_symbol}2500')",
    ...
    "individual_stock_recommendations": [
        {{
            ...
            "investment_amount": "string (NEVER {currency_symbol}0 for BUY, always {currency_symbol}0 for HOLD/SELL)",
...
- NEVER EVER return {currency_symbol}0 for BUY recommendations - if its a BUY recommendation then amount has to be allocated else it will be HOLD recommendation.
-- If a stock is BUY recommendation but {currency_symbol}0 investment then it will be put to HOLD if it is part of `existing_stocks`
-- If a stock is BUY recommendation but {currency_symbol}0 investment then it will be removed from recommendation if it is part of new_stocks
...
- If there is not enough stocks to buy and reach the full {currency_symbol}{self.investment_amount} then its okay to invest less than {currency_symbol}{self.investment_amount}
- Ensure total BUY allocations sum does not exceed {currency_symbol}{self.investment_amount}
...
- **CURRENCY**: All monetary values MUST use {currency_symbol} symbol (not $ or any other symbol)

VALIDATION CHECKLIST (Before returning JSON):
✓ All BUY recommendations have investment_amount > {currency_symbol}0 (minimum 5% of budget)?
✓ All HOLD/SELL recommendations have investment_amount = "{currency_symbol}0"?
...
✓ Total of all BUY investment amounts = {currency_symbol}{self.investment_amount}?
✓ No single stock allocation > 25% of budget?
✓ All JSON fields properly formatted with correct types?
✓ All monetary values use {currency_symbol} symbol?"""
                    ...
                    if response_text:
                        ...
                        try:
                            ...
                            for idx, stock_rec in enumerate(individual_recommendations):
                                ...
                                # Extract numeric value from investment_amount (handle both $ and ₹ symbols)
                                amount_value = 0.0
                                if isinstance(investment_amount, str):
                                    amount_str = investment_amount.replace("$", "").replace("₹", "").replace(",", "").strip()
                                    try:
                            ...
                            if len(buy_recommendations) >= 3:  # Only validate if 3+ BUY recommendations
                                # Extract investment amounts (handle both $ and ₹ symbols)
                                buy_amounts = []
                                for rec in buy_recommendations:
                                    amount_str = rec.get("investment_amount", "$0").replace("$", "").replace("₹", "").replace(",", "").strip()
                                    try:
                            ...
                            for allocation in parsed_json["allocation_breakdown"]:
                                amount_str = allocation.get("investment_amount", "$0").replace("$", "").replace("₹", "").replace(",", "").strip()
                                try:
            ...
            try:
                ...
                for stock_rec in individual_recommendations:
                    ...
                    if recommendation in ["BUY", "SELL"] and ticker in entry_prices:
                        ...
                        stock_rec["entry_price"] = f"{currency_symbol}{current_price:.2f}"
                        logger.info(f"Added entry price {currency_symbol}{current_price:.2f} to {recommendation} recommendation for {ticker}")
                        prices_added += 1
                        ...
                        if recommendation == "BUY":
                            ...
                            try:
                                # Parse investment amount (remove currency symbol and convert to float)
                                investment_value = float(investment_amount_str.replace("$", "").replace("₹", "").replace(",", ""))
                ...
                # Add entry prices, recommendation timestamp, and market preference to the recommendations
                recommendations_dict["entry_prices"] = entry_prices
                recommendations_dict["recommendation_date"] = datetime.now().isoformat()
                recommendations_dict["market_preference"] = self.market_preference
                recommendations_dict["currency_symbol"] = currency_symbol
                logger.info(f"Added entry prices for {len(entry_prices)} stocks to recommendation (market: {self.market_preference}, currency: {currency_symbol})")
