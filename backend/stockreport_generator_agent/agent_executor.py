...
logger.info("StockReportGeneratorAgentExecutor initialized with centralized logging")


class StockReportGeneratorAgentExecutor(AgentExecutor):
    ...
    def _run_agent(
        ...
        return self.runner.run_async(
            session_id=session_id, user_id="stock_report_generator_agent", new_message=new_message
        )
    ...
    async def _process_request(
        ...
        agen = self._run_agent_with_retry(session_id, new_message)
        try:
            ...
            try:
                ...
                logger.warning(f"Error closing agent event generator for session {session_id}: {close_error}")
    ...
    async def _upsert_session(self, session_id: str):
        session = await self.runner.session_service.get_session(
            app_name=self.runner.app_name, user_id="stock_report_generator_agent", session_id=session_id
        )
        if session is None:
            session = await self.runner.session_service.create_session(
                app_name=self.runner.app_name,
                user_id="stock_report_generator_agent",
